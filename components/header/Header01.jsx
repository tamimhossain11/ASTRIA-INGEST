...
import Logo from "./../../public/images/logo_white.jpg";
import WhiteLogo from "./../../public/images/logo_white.jpg";
...
export default function Header01() {
  ...
  const cases = {
    ...
    pages: [
      ...
      {
        id: uuidv4(),
        name: "Upload Evidance",
        path: "/upload_evidance",
      },
    ],
  };
  ...
  return <>
    ...
    <header className="js-page-header fixed top-0 z-20 w-full backdrop-blur transition-colors">
      <div className="flex items-center px-6 py-6 xl:px-24 ">
        ...
        <form
          action="search"
          className="relative ml-12 mr-2 hidden basis-2.5/12 lg:block xl:ml-[7%]"
        >
          <input
            type="search"
            className="text-jacarta-700 placeholder-jacarta-500 focus:ring-accent border-jacarta-100 w-full rounded-3xl border py-[0.6875rem] px-4 pl-10 dark:border-transparent bg-black-900 dark:bg-white/[.15] dark:text-white placeholder-white"
            placeholder="Search"
          />
            ...
            <ul className="flex flex-col lg:flex-row">
              ...
              <li className="js-nav-dropdown group relative">
                <button className="dropdown-toggle text-jacarta-700 font-display hover:text-accent focus:text-accent dark:hover:text-accent dark:focus:text-accent flex items-center justify-between py-3.5 text-base dark:text-white lg:px-5 w-full">
                  <span
                    className={
                      isParentPageActive(cases.pages, route.asPath)
                        ? "text-accent dark:text-accent"
                        : ""
                    }
                  >
    ...
    >
      ...
      <nav className="navbar w-full">
        ...
        <li className="group" onClick={() => setToggle(false)}>
          ...
          <li className="js-nav-dropdown group relative">
            <button
              onClick={() => mobileCollapse(cases.id)}
            ...
            <ul
              className={`dropdown-menu left-0 top-[85%] z-10 grid-flow-row grid-cols-[repeat(2,_1fr)] gap-x-4 whitespace-nowrap rounded-xl bg-white transition-all will-change-transform group-hover:visible group-hover:opacity-100 dark:bg-jacarta-800 lg:invisible lg:absolute lg:!grid lg:translate-y-4 lg:py-8 lg:px-2 lg:opacity-0 lg:shadow-2xl lg:group-hover:translate-y-2 relative ${
                isCollapse === cases.id ? "block" : "hidden"
              }`}
              aria-labelledby="navDropdown-4"
            >
              {cases?.pages?.map((page) => (
